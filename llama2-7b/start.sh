...
function log_system_info() {
  ...
  if command -v nvidia-smi &> /dev/null; then
    ...
    echo "No NVIDIA GPU or drivers detected" >> "$SYSTEM_INFO"
  fi
...
if command -v ollama &> /dev/null; then
  ...
  echo "Ollama is not installed or not in PATH" >> "$SYSTEM_INFO"
fi
...
function check_ollama_service() {
  ...
  if ! $is_running; then
    log_error "Ollama service did not become ready within the expected time"
    return 1
  fi
...
curl -s "http://${HOST}:${PORT}/api/generate" \
    ...
    jq -r '.response' 2>/dev/null || echo "Request failed or result parsing error"
